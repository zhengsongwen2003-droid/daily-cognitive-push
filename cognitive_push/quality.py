...
SECTION_PATTERN = re.compile(r"(?m)^(?P<header>[1-7]\. [^\n]+)\s*$")
...
def _section_bodies(card: str) -> dict[str, str]:
    ...
    headers = [match.group("header") for match in matches]
    ...
    for index, match in enumerate(matches):
        ...
        bodies[match.group("header")] = card[start:end].strip()
    return bodies
...
def validate_card(card: str, recent_titles: set[str], recent_themes: set[str]) -> QualityResult:
    ...
    section_headers = [match.group("header") for match in SECTION_PATTERN.finditer(card)]
    ...
    if "10 分钟" not in action_body and "十分钟" not in action_body:
